...
local Anchor = CreateEmptyWindow("Anchor", "UIParent")
      Anchor:Show(true)
      Anchor:AddAnchor("CENTER", "UIParent", 0,0)
...
local function CreateTextButton()
    ...
    function textButton:OnClick()
        ...
        if displayCommerce ~= 0 and language == "zh_cn" then
...
if savedWindowX ~= nil and savedWindowY ~= nil then
    local uiScale = UIParent:GetUIScale() or 1.0
    Anchor:AddAnchor("TOPLEFT", "UIParent", savedWindowX*uiScale,savedWindowY*uiScale)
    X2Chat:DispatchChatMessage(CMF_SYSTEM, string.format("成功加载货率插件位置信息!x:%s,y:%s",tostring(savedWindowX*uiScale) or "error",tostring(savedWindowY*uiScale) or "error"))
else
    X2Chat:DispatchChatMessage(CMF_SYSTEM, string.format("未能加载货率插件位置信息，默认出现在屏幕中央"))
end
...
local packRatioListnerWindow = CreateEmptyWindow("packRatioListnerWindow", "UIParent")
packRatioListnerWindow:Show(false)
packRatioListnerWindow:SetHandler("OnEvent", function(this, event, ...)
  packRatioChartListener[event](...)
end)
...
RegistUIEvent(packRatioListnerWindow, packRatioChartListener)